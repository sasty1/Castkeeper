...
function CastKeeperApp() {
  ...
  const handleServerSchedule = async () => {
    ...
    try {
      ...
      if (delaySeconds < 1) {
         setStatus({msg: "Please select a future time.", type: 'error'});
         setLoading(false);
         return;
      }
      ...
      if (data.success) {
        setStatus({msg: 'Scheduled! Safe to close app.', type: 'success'});
        setText('');
        setTargetDate('');
        setIsScheduled(false);
      } else {
        setStatus({msg: "Schedule Error: " + data.error, type: 'error'});
      }
    } catch (e) {
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#0a0a0a] text-center p-6 relative overflow-hidden font-sans z-50">
        <div className="z-10 max-w-md w-full space-y-10">
          <div className="space-y-4">
            ...
            <p className="text-[#888] text-lg leading-relaxed px-4">The pro scheduler for Farcaster.</p>
          </div>
          <div className="w-full px-2 flex justify-center">
             <button onClick={handleNativeLogin} className="w-full bg-[#5E5CE6] hover:bg-[#4d4bbd] text-white font-semibold py-4 rounded-2xl shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2">
               <FarcasterIcon />
      ...
      <div className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl p-1 shadow-2xl">
          <div className="bg-black/40 rounded-xl p-5 space-y-4">
             ...
             {loading && approvalUrl && !signerUuid && (
                <button onClick={() => sdk.actions.openUrl(approvalUrl)} className="w-full text-xs text-yellow-400 hover:text-yellow-300 underline flex items-center justify-center py-2"><LinkIcon /> Tap here if approval screen didn't open</button>
             )}
          ...
          <div className="space-y-2 max-h-[200px] overflow-y-auto custom-scrollbar">
            {drafts.map((draft) => (
              <div key={draft.id} onClick={() => setText(draft.text)} className="group flex justify-between items-center p-4 bg-white/5 hover:bg-white/10 border border-white/5 hover:border-white/10 rounded-xl cursor-pointer transition-all">
                ...
                <button onClick={(e) => { e.stopPropagation(); const newDrafts = drafts.filter((d) => d.id !== draft.id); setDrafts(newDrafts); localStorage.setItem("drafts_" + user.fid, JSON.stringify(newDrafts)); }} className="text-gray-600 hover:text-red-400 p-2 opacity-0 group-hover:opacity-100 transition-opacity"><TrashIcon /></button>
              </div>
            ))}
          </div>
        </div>
