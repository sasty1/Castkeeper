...
function CastKeeperApp() {
  const { user, signIn } = useNeynarContext(); // Using manual signIn for better control
  ...
  // --- 1. TELL WARPCAST WE ARE READY (Fixes Splash Screen) ---
  useEffect(() => {
    const load = async () => {
      sdk.actions.ready(); 
    };
  ...
  // --- LOGIN SCREEN (Custom Button Fix) ---
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#0a0a0a] text-center p-6 relative overflow-hidden font-sans z-50">
        ...
        <div className="z-10 max-w-md w-full space-y-10">
          ...
          <div className="w-full px-2 flex justify-center">
             {/* MANUAL BUTTON - Fixes Android Redirects */}
             <button 
               onClick={() => signIn()}
               className="w-full bg-[#5E5CE6] hover:bg-[#4d4bbd] text-white font-semibold py-4 rounded-2xl shadow-lg transition-all active:scale-95 flex items-center justify-center"
             >
               <FarcasterIcon />
